 ...
 */
router.put("/update/:postSubcategorieId", (0, requireDashboardPermission_1.requirePermission)("M_Blog"), multer_1.memoryUpload.fields([
    ...
    try {
        // 1) load existing
        const existing = yield PostSubCategorie_1.default.findById(postSubcategorieId);
        if (!existing) {
        ...
        if (iconFile) {
            ...
            const uploadedIcon = yield (0, uploadToCloudinary_1.uploadToCloudinary)(iconFile, "postSubCategories");
        ...
        if (imageFile) {
            ...
            const uploadedImage = yield (0, uploadToCloudinary_1.uploadToCloudinary)(imageFile, "postSubCategories");
        ...
        if (bannerFile) {
            ...
            const uploadedBanner = yield (0, uploadToCloudinary_1.uploadToCloudinary)(bannerFile, "postSubCategories");
            updateData.bannerUrl = uploadedBanner.secureUrl;
            updateData.bannerId = uploadedBanner.publicId;
        }
        // 6) apply update
        const updated = yield PostSubCategorie_1.default.findByIdAndUpdate(postSubcategorieId, updateData, { new: true, runValidators: true });
        if (!updated) {
            res.status(404).json({
                success: false,

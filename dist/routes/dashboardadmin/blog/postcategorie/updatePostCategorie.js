 ...
 */
router.put("/update/:postCategorieId", (0, requireDashboardPermission_1.requirePermission)("M_Blog"), multer_1.memoryUpload.fields([
    ...
    try {
        // 1) load existing
        const existing = yield PostCategorie_1.default.findById(postCategorieId);
        if (!existing) {
        ...
        if (iconFile) {
            ...
            const uploadedIcon = yield (0, uploadToCloudinary_1.uploadToCloudinary)(iconFile, "postCategories");
        ...
        if (imageFile) {
            ...
            const uploadedImage = yield (0, uploadToCloudinary_1.uploadToCloudinary)(imageFile, "postCategories");
        ...
        if (bannerFile) {
            ...
            const uploadedBanner = yield (0, uploadToCloudinary_1.uploadToCloudinary)(bannerFile, "postCategories");
            updateData.bannerUrl = uploadedBanner.secureUrl;
            updateData.bannerId = uploadedBanner.publicId;
        }
        // 6) apply update
        const updated = yield PostCategorie_1.default.findByIdAndUpdate(postCategorieId, updateData, { new: true, runValidators: true });
        if (!updated) {
            res
                .status(404)
